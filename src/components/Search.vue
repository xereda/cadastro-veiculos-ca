<template>
  <nav class="level">
    <div class="level-left" v-show="filter.length === 0">
      <div class="level-item is-hidden-mobile">
        <p class="subtitle is-5">
          <strong>{{ totalBeers }}</strong> Beers
        </p>
      </div>
      <div class="level-item">
        <div class="field has-addons">
          <p class="control">
            <input :class="{ 'input': true, 'is-disabled': false }" @keyup.enter="setFilter()" v-model="searchString" type="text" placeholder="Pesquise um beer">
          </p>
          <p class="control">
    ...
    <div class="level-right is-hidden-mobile">
      ...
      <p class="level-item"><a>Beers</a></p>
